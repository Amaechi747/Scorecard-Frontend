...
const AdminProfile = (props: PageProps): JSX.Element => {
    ...
    const handleImageChange = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.currentTarget.files[0]) {
            try {
                // @ts-ignore: Object is possibly 'null'.
                const result = await axios.put(`/admin/upload/${localStorage.getItem('id')}`, {image: e.currentTarget.files[0]}, {
                    headers: {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            const result = await axios.patch(`/admin/edit/${localStorage.getItem('id')}`, {firstName, lastName}, {
                headers: {
                    'authorization': `Bearer ${localStorage.getItem('token')}`
                }
            })
    ...
    useEffect(() => {
            axios.get(`/admin/profile/${localStorage.getItem('id')}`,{
                headers: {
                  'authorization': `Bearer ${localStorage.getItem('token')}`
                }
              })
        ...
        <>
          <Heading>Profile</Heading> 
            <Card2>
                ...
                <ProfileImage>
                    <div>
                        {/** use State and conditional logic to trigger */
